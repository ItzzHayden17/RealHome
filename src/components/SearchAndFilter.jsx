...
const SearchAndFilter = (props) => {
  ...
  var filteredData = useMemo(() => {
    return propertiesData.filter((data) => {
      return (
        (filters.suburb == "" ||
          data.suburb.toLowerCase().includes(filters.suburb.toLowerCase())) &&
        (filters.type == "" ||
          data.type.toLowerCase() == filters.type.toLowerCase()) &&
        (filters.city == "" ||
          data.city.toLowerCase() == filters.city.toLowerCase()) &&
        (filters.price == "" ||
          (data.price >= Number(filters.price) &&
            data.price <= Number(filters.price) + 50000)) &&
